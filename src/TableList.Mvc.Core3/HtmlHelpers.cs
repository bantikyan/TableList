...
namespace Zetalex.TableList.Mvc.Core3
{
    public static class HtmlHelpers
    {
        ...
        public static IHtmlContent TableListFor<TModel, TProperty>(this IHtmlHelper<TModel> html, Expression<Func<TModel, TProperty>> expression, IDictionary<string, object> htmlAttributes, bool showHeader = false, bool allowAdd = true)
        {
            ...
            var fieldName = expresionProvider.GetExpressionText(expression);

            var metadataModel = html.ViewData.Model.GetType().GetProperty(fieldName).GetValue(html.ViewData.Model, null);
            var metadataModelType = html.ViewData.Model.GetType().GetProperty(fieldName).PropertyType;

            dynamic model = metadataModel;
            Type baseType = metadataModelType.GetGenericArguments().Single();
        ...
        private static string GetErrorMessage(ModelMetadata modelMetadata)
        {
            ...
            var modelBindingMessageProvider = modelMetadata.ModelBindingMessageProvider;
            string text = modelMetadata.DisplayName ?? modelMetadata.PropertyName;
            if (text == null)
            {
                return modelBindingMessageProvider.NonPropertyValueMustBeANumberAccessor();
            }
            return modelBindingMessageProvider.ValueMustBeANumberAccessor(text);
        }
    }
}
